...
- `def refused_symlink` — Name the symlinked marker :func:`resolve_config` refused, or ``""``; never raises.
...
- `class ContractProbe` — The contract-version half of a freshness run (#221).
- `def parse_contract_version` — The ``CONTRACT_VERSION`` a project-init source file declares; ``None`` if none (pure).
- `def compare_contract_version` — Flag an upstream descriptor contract newer than this orchestrator understands (pure).
...
- `def fetch_upstream_contract_source` — Fetch the project-init file that declares its contract version; ``None`` on any problem.
...
- `def origin_repo` — ``owner/name`` for ``repo``'s ``origin`` remote; ``""`` when it is not GitHub's.
...
- `def render_heal_message` — The human-readable text of a heal notification (pure).
...
- `def published_default_head` — The commit ``origin``'s default branch points at NOW; ``""`` when unknown.
