...
allowed-tools: Bash(gh *) Bash(.agents/scripts/*) Read
---
...
- Use `.agents/scripts/create_issue.sh`; do not call `gh issue create` directly unless the script cannot satisfy the case.
...
```bash
.agents/scripts/create_issue.sh <type> "<title>" \
