...
effort: high
allowed-tools: Bash(git *) Bash(gh *) Bash(.agents/scripts/*) Read
---
...
3. **Finish — push, mark ready, and merge:**
   ```bash
   .agents/scripts/finish_pr.sh [pr-number]
   ```
   ...
   ```bash
   .agents/scripts/push_branch.sh
   .agents/scripts/promote_review.sh [pr-number]
   .agents/scripts/monitor_pr.sh <pr-number> --merge
   ```
...
1. Post a response for each review comment:
   ...
   ```
2. Fix actionable code, then push: `.agents/scripts/push_branch.sh`
3. Re-run with the next cycle number:
   ```bash
   .agents/scripts/monitor_pr.sh <pr-number> --merge --review-cycle <N>
   ```
