...
- **This repository is PUBLIC** — a commit, an issue and a PR body all publish, and most other trees on this machine are private. Never publish, in code or in issue/PR/commit text: a client or account name, a person's or role's email address, an absolute filesystem path from this machine, or content from an agent's private configuration directory. The exception is this package's own published metadata — `pyproject.toml`'s `authors` field carries the maintainer's name and email deliberately because PyPI requires it, and that must not be "fixed". The test is whether a fact is public **by intent**, not whether it is on disk; `git grep` over tracked files is the check, because an untracked scratch file is not the risk and a tracked one is. The same text is delivered at session start by [`.claude/inject.d/10-this-repo-is-public.md`](.claude/inject.d/10-this-repo-is-public.md), which Claude loads and nothing else does — so this bullet is the canonical copy and the pack is the convenience.
