...
- **Agent support tiers** — only the Claude Code path is functionally CI-tested. Codex: skills are discoverable under `.agents/skills/` and the command + destructive-command (`prod_guard`) guards are wired via `.codex/hooks.json` (adapter: `.claude/hooks/agent_guard_adapter.py`) — **advisory**: some Codex versions gate project-scoped hooks behind a one-time trust/enable step before they fire, so git + CI remain the real boundary. Codex/Antigravity overlays are validated by contract tests on the rendered files, not by running those agents; the real security boundary for every agent is the git/CI enforcement above.
...
- Hooks fire automatically via the `project-init-workflow` plugin (enabled in `.claude/settings.json` under `enabledPlugins`, not a local `hooks` block): `pre_commit_gate` (lint gate on commit), `github_command_guard` (steers toward lifecycle scripts), `post_edit_lint`, `workflow_state_reminder`, `prod_guard`. The wired scripts live under the plugin's `CLAUDE_PLUGIN_ROOT`/hooks; `.claude/hooks/` keeps only the libraries the lifecycle scripts call (`dag_workflow.py`, `prod_guard.py`, `_py.sh`). To see or change the wiring, edit the plugin, not `settings.json`.
