...
#: The evidence quoted in an issue is a tail, not a log: enough to see the
#: failure, and far below GitHub's body limit.
...
def _redact(text: str) -> str:
    """Replace the home directory with ``~``: the issue may land in a public repository."""
    home = str(Path.home())
...
def issue_body(
    ...
    lines = [
        ...
        f"- Checked: {when}{at}",
        "",
        "Evidence (the tail of the gate's output):",
        "",
        _fenced(evidence),
        "",
...
def _deliver_project(
    ...
    for task in to_file:
        ...
        if not marker:
            ...
            ok = False
            continue
        body = issue_body(descriptor, cached[task], notified.detail if notified else "", marker)
    ...
    for task in passing:
        ...
        if closed.ok:
            ...
            ok = False
    return ok if (wrote or not ok) else None
