...
# --- The optional `ci:` block (project-init #828, released in 1.1.7) ---
# An ADDITIVE field within contract v2: project-init emits it, the contract
# version does NOT move, and a child that predates it must keep working. It rides
# the same golden v2 fixture as everything else — real `project-init
# --non-interactive --json` output, never hand-written, so a producer that drops
# the block fails here instead of the adapter silently never firing on a fleet.


def test_producer_emits_the_ci_block() -> None:
    raw = yaml.safe_load(_CONFIG_V2.read_text(encoding="utf-8"))
...
def test_scaffold_default_leaves_ci_unset_so_the_forge_still_probes(tmp_path: Path) -> None:
    ...
    assert _descriptor_v2(tmp_path).ci is None
