...
def silent_sites(source: str) -> list[int]:
    ...
    A handler leaves a trail when it re-raises or USES the exception it bound —
    logging it, or carrying it into a warning or a message. Otherwise it must say,
    in an ``# expected:`` comment between its header and its first statement, why
    the exception IS the answer rather than a fault. ``contextlib.suppress`` is the
    same swallow in another spelling, so it answers to the same rule.
    """
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ExceptHandler):
            ...
            uses = node.name is not None and any(
                isinstance(n, ast.Name) and n.id == node.name
                for stmt in node.body
                for n in ast.walk(stmt)
            )
            if not (raises or uses or _marked(comments, node.lineno, node.body[0].lineno)):
