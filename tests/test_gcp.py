...
def test_the_only_command_is_read_only() -> None:
    ...
    assert "search-all-resources" in gcp.SEARCH_COMMAND
    lowered = gcp.SEARCH_COMMAND.lower()
    assert not any(token in lowered for token in _MUTATING_TOKENS)
...
def test_search_resources_issues_only_the_read_only_command(
    ...
    def spy(command: str, **_kwargs: object) -> RunResult:
        ...
        return RunResult(command=gcp.SEARCH_COMMAND, returncode=0, stdout="[]")

    monkeypatch.setattr(gcp, "run_command", spy)
    gcp.search_resources()
    assert issued == [gcp.SEARCH_COMMAND]
...
def test_search_resources_parses_the_inventory(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        gcp,
        "run_command",
        lambda *_a, **_k: RunResult(command=gcp.SEARCH_COMMAND, returncode=0, stdout=payload),
    )
    resources = gcp.search_resources()
...
def test_search_resources_is_unknown_when_the_scan_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: RunResult(
            command=gcp.SEARCH_COMMAND, returncode=1, stderr="ERROR: unauthenticated"
        ),
    )
    assert gcp.search_resources() is None
...
def test_search_resources_is_unknown_when_gcloud_is_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ...
        "run_command",
        lambda *_a, **_k: RunResult(
            command=gcp.SEARCH_COMMAND, returncode=None, error="gcloud: not found"
        ),
    )
    assert gcp.search_resources() is None
...
def test_search_resources_is_unknown_on_non_json_output(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ...
        "run_command",
        lambda *_a, **_k: RunResult(
            command=gcp.SEARCH_COMMAND, returncode=0, stdout="not json at all"
        ),
    )
    assert gcp.search_resources() is None
...
def test_search_resources_is_unknown_when_output_is_not_a_list(
    ...
    monkeypatch.setattr(
        ...
        "run_command",
        lambda *_a, **_k: RunResult(
            command=gcp.SEARCH_COMMAND, returncode=0, stdout='{"error": "x"}'
        ),
    )
    assert gcp.search_resources() is None
