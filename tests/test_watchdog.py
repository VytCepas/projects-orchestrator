...
def test_a_future_timestamp_does_not_read_as_negative_age(tmp_path: Path) -> None:
    """A clock correction or a bad RTC stamps the heartbeat ahead of now.
    Unclamped, the age goes negative and the pass reads as freshly-run for as
    long as the skew lasts — a dead monitor hidden by arithmetic."""
    path = tmp_path / "watch.json"
    _at(path, ago=-_HOUR * 50, interval=_HOUR)
    assert watchdog.read_state(path, now=_NOW).age_seconds == 0
